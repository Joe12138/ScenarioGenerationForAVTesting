...
class SimpleMergeToLessRoad:
    ...
    def road_generation(self) -> Road:
        ...
        for lane_sec_idx in range(3):
            # if isinstance(self.lane_length, list):
            #     s = self.lane_length[lane_sec_idx]
            # else:
            #     s = lane_sec_idx * self.lane_length/3
            ...
            for r_idx in range(right_lane_num):
                if lane_sec_idx == 1:
                    ...
                    right_lane = Lane(a=self.left_lane_width[r_idx])
                if lane_sec_idx == 2:
                    if right_add_one:
        ...
        # for link in lanelinker.links:
        #     print(link)
        lanes = Lanes()
        lanes.add_lanesection(lanesection=start_lane_section, lanelinks=lanelinker)
        lanes.add_lanesection(lanesection=middle_lane_section, lanelinks=lanelinker)
        lanes.add_lanesection(lanesection=end_lane_section, lanelinks=lanelinker)
        
        road = Road(road_id=self.road_id, planview=planview, lanes=lanes)
